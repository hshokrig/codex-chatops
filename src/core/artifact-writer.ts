...
export class ArtifactWriter {
  ...
  async writeSessionArtifacts(session: SessionRecord): Promise<void> {
    ...
    await writeTextFile(
      path.join(root, "branch.txt"),
      `${session.branchName}\n`
    );
    await writeTextFile(
      path.join(root, "worktree.txt"),
  ...
  async prepareAttachments(
    ...
    for (const attachment of attachments) {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to download attachment ${attachment.name}: ${response.status} ${response.statusText}`
        );
      }

      const filePath = path.join(
        this.attachmentRoot(sessionId, runId),
        `${attachment.id}-${sanitizeFileName(attachment.name)}`
      );
  ...
  async writeUsageRollup(rollup: UsageRollup): Promise<void> {
    const filePath = path.resolve(
      this.chatopsRoot,
      "usage",
      `${rollup.date}-${rollup.repoSlug}.json`
    );
